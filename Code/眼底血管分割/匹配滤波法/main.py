...
def build_filters2(sigma=1, YLength=10):
    ...
    for theta in np.arange(0, np.pi, np.pi / 16):
        # theta = np.pi/4
        matchFilterKernel = np.zeros((widthOfTheKernel, widthOfTheKernel), dtype=float)
...
def grayStretch(img, m=60.0/255, e=8.0):
    k = np.zeros(img.shape, float)
    ans = np.zeros(img.shape, float)
    mx = np.max(img)
    mn = np.min(img)
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
...
if __name__ == '__main__':
    ...
    srcImg = cv2.imread('/Users/larry/Documents/GitHub/Medical-Image-Segmentation/DataSets/DRIVE_png/training/image/21_training.png', cv2.IMREAD_ANYDEPTH | cv2.IMREAD_ANYCOLOR)

    # 标定图
    # grountruth = cv2.imread(path + ('%02d' % num) + '_manual1.tif', cv2.IMREAD_GRAYSCALE)
    grountruth = cv2.imread('/Users/larry/Documents/GitHub/Medical-Image-Segmentation/DataSets/DRIVE_png/training/manual/21_manual1.png', cv2.IMREAD_GRAYSCALE)
    ...
    # print(num, '', dice)
    wtf = np.hstack(
        [srcImg, cv2.cvtColor(grountruth, cv2.COLOR_GRAY2BGR), cv2.cvtColor(predictImg, cv2.COLOR_GRAY2BGR)])
    cv2.imwrite('test1.png', wtf)
    # cv2.waitKey()
